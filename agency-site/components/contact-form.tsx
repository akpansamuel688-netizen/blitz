...
export function ContactForm() {
  ...
  const FieldError=({name}:{name:keyof FormState})=>errors[name]?<p className="mt-1 text-xs text-red-600">{errors[name]}</p>:null;
  return <form onSubmit={submit} noValidate className="grid gap-5 rounded-[28px] border border-slate-200 bg-white p-5 shadow-xl shadow-slate-200/50 sm:p-8">
    <div className="grid gap-5 sm:grid-cols-2">
      <label className="text-sm font-bold">Full Name *<input className={fieldClass} value={form.fullName} onChange={e=>update("fullName",e.target.value)} autoComplete="name"/><FieldError name="fullName"/></label>
      <label className="text-sm font-bold">Email *<input className={fieldClass} type="email" value={form.email} onChange={e=>update("email",e.target.value)} autoComplete="email"/><FieldError name="email"/></label>
      ...
      <label className="text-sm font-bold">Type of Project *<select className={fieldClass} value={form.projectType} onChange={e=>update("projectType",e.target.value)}><option value="">Select a project</option>{projectTypes.map(x=><option key={x}>{x}</option>)}</select><FieldError name="projectType"/></label>
      <label className="text-sm font-bold">Service Needed *<select className={fieldClass} value={form.service} onChange={e=>update("service",e.target.value)}><option value="">Select a service</option>{serviceOptions.map(x=><option key={x}>{x}</option>)}</select><FieldError name="service"/></label>
