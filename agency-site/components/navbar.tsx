...
export function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-white/10 bg-[#101915]/90 text-[#e7efe9] backdrop-blur-xl">
    <nav className="container-shell flex h-[76px] items-center justify-between" aria-label="Primary navigation">
      <Link href="#home" className="flex items-center gap-3 font-extrabold tracking-tight" onClick={()=>setOpen(false)}>
        <span className="grid size-9 place-items-center rounded-xl bg-emerald-300 text-sm text-[#101915] shadow-lg">M</span>
      ...
      <button className="grid size-11 place-items-center rounded-full border border-white/15 bg-white/5 lg:hidden" onClick={()=>setOpen(!open)} aria-expanded={open} aria-controls="mobile-menu" aria-label={open ? "Close menu" : "Open menu"}>{open?<X/>:<Menu/>}</button>
    </nav>
    {open && <div id="mobile-menu" className="fixed inset-x-0 top-[76px] h-[calc(100vh-76px)] border-t border-white/10 bg-[#101915] px-5 py-8 lg:hidden">
