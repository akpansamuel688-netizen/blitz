...
export default function Home() {
  return <main>
    ...
    <section className="section bg-[#eef5ff]">
    ...
    <section id="technologies" className="section border-y border-slate-200 bg-[#f8fbff]">
      <div className="container-shell">
    ...
    <section id="about" className="section bg-white">
      <div className="container-shell grid gap-14 lg:grid-cols-2 lg:items-center">
        ...
        <div className="relative rounded-[32px] bg-[#edf4ff] p-7 sm:p-10"><div className="absolute right-0 top-0 size-40 rounded-full bg-cyan-300/20 blur-3xl"/><p className="relative text-sm font-bold text-blue-600">THE MONARK STANDARD</p><div className="relative mt-7 grid gap-4 sm:grid-cols-2">{["Business-aware decisions","Excellent user experience","Maintainable foundations","Measurable performance"].map((x,i)=><div key={x} className="rounded-2xl bg-white p-5 shadow-sm"><span className="grid size-8 place-items-center rounded-full bg-slate-950 text-cyan-300"><Check size={15}/></span><p className="mt-5 font-extrabold">{x}</p><p className="muted mt-2 text-xs leading-5">{["We connect technical choices to real goals.","We make complex products feel simple.","We build for the team that inherits the code.","We treat speed and quality as product features."][i]}</p></div>)}</div></div>
      </div>
    </section>

    <section id="contact" className="section border-t border-slate-200 bg-[#f1f6fd]">
